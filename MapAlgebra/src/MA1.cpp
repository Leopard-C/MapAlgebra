...
{
	Bmp bmpSrc1, bmpSrc2;
	bmpSrc1.read(bmpName1);
	bmpSrc2.read(bmpName2);

	if (!bmpSrc1.add(bmpSrc2, alpha))
		return false;

	bmpSrc1.write(outputBmpName);
...
{
	Bmp bmpSrc1, bmpSrc2;
	BmpThreshold(bmpName1, "data/temp/thres-temp.bmp", 0xFF);
	bmpSrc1.read("data/temp/thres-temp.bmp");
	BmpThreshold(bmpName2, "data/temp/thres-temp.bmp", 0xFF);
	bmpSrc2.read("data/temp/thres-temp.bmp");

	if (!Bmp::isSameSize(bmpSrc1, bmpSrc2) || bmpSrc1.bitCount() != 8) {
		LOG("Not same size or bit count");
		return false;
	}
	...
	for (int y = 0; y < height; ++y) {
		for (int x = 0; x < width; ++x) {
			if (bmpSrc1.mtx[y][x] != 0xFF && bmpSrc2.mtx[y][x] != 0xFF)
				bmpSrc1.mtx[y][x] = 0x00;
			else
				bmpSrc1.mtx[y][x] = 0xFF;
		}
	}
...
bool BmpBooleanNot(const char* bmpName, const char* outputBmpName)
{
	...
	for (int y = 0; y < height; ++y) {
		for (int x = 0; x < width; ++x) {
			bmpSrc.mtx[y][x] = 0xFF - bmpSrc.mtx[y][x];
		}
	}

	bmpSrc.write(outputBmpName);
...
bool BmpBooleanXOr(const char* bmpName1, const char* bmpName2, const char* outputBmpName)
{
	...
	for (int y = 0; y < height; ++y) {
		for (int x = 0; x < width; ++x) {
			if (bmpSrc1.mtx[y][x] != bmpSrc2.mtx[y][x])
				bmpSrc1.mtx[y][x] = 0x00;
			else
				bmpSrc1.mtx[y][x] = 0xFF;
		}
...
bool BmpSmooth(const char* srcBmpName, const char* destBmpName)
{
	...
	int width = bmpSrc.width();
	int height = bmpSrc.height();
	uchar** mtx = bmpSrc.mtx;

	switch (bmpSrc.bitCount()) {
	default:
		break;
	case 8:
	{
		Bmp bmpDest(8, width, height);
		bmpDest.copyClrTabFrom(bmpSrc.clrTab, 1024);
		bmpDest.copyMtxFrom(bmpSrc.mtx, bmpSrc.mtxWidth, bmpSrc.mtxHeight);

		// 周围9个像素点的平均值
		for (int y = 1; y < height - 1; ++y) {
			for (int x = 1; x < width - 1; ++x) {
				bmpDest.mtx[y][x] = (mtx[y - 1][x - 1] + mtx[y - 1][x]
					+ mtx[y - 1][x + 1] + mtx[y][x - 1]
					+ mtx[y][x] + mtx[y][x + 1]
					+ mtx[y + 1][x - 1] + mtx[y + 1][x]
					+ mtx[y + 1][x + 1]) / 9;
			}
		}
		bmpDest.write(destBmpName);
	...
	case 32:
	{
		Bmp bmpDest(32, width, height);
		bmpDest.copyMtxFrom(bmpSrc.mtx, bmpSrc.mtxWidth, bmpSrc.mtxHeight);

		// 周围9个像素点的平均值
		for (int y = 1; y < height - 1; ++y) {
			for (int x = 1; x < width - 1; ++x) {
				bmpDest.mtx[y][x * 4] = (mtx[y - 1][(x - 1) * 4] + mtx[y - 1][x * 4]
					+ mtx[y - 1][(x + 1) * 4] + mtx[y][(x - 1) * 4]
					+ mtx[y][x * 4] + mtx[y][(x + 1) * 4]
					+ mtx[y + 1][(x - 1) * 4] + mtx[y + 1][x * 4]
					+ mtx[y + 1][(x + 1) * 4]) / 9;
				bmpDest.mtx[y][x * 4 + 1] = (mtx[y - 1][(x - 1) * 4 + 1] + mtx[y - 1][x * 4 + 1]
					+ mtx[y - 1][(x + 1) * 4 + 1] + mtx[y][(x - 1) * 4 + 1]
					+ mtx[y][x * 4 + 1] + mtx[y][(x + 1) * 4 + 1]
					+ mtx[y + 1][(x - 1) * 4 + 1] + mtx[y + 1][x * 4 + 1]
					+ mtx[y + 1][(x + 1) * 4 + 1]) / 9;
				bmpDest.mtx[y][x * 4 + 2] = (mtx[y - 1][(x - 1) * 4 + 2] + mtx[y - 1][x * 4 + 2]
					+ mtx[y - 1][(x + 1) * 4 + 2] + mtx[y][(x - 1) * 4 + 2]
					+ mtx[y][x * 4 + 2] + mtx[y][(x + 1) * 4 + 2]
					+ mtx[y + 1][(x - 1) * 4 + 2] + mtx[y + 1][x * 4 + 2]
					+ mtx[y + 1][(x + 1) * 4 + 2]) / 9;
			}
		}
		bmpDest.write(destBmpName);
